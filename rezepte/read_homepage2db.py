...
rz.create_recipie_db('rezepte.sqlite')
...
for navlink in navlinks:
    ...
    with open(base_url + navlink.get('href')) as html_file:
        ...
        for i in range(1, len(rezepte)):
            # with open(base_url + link.get('href')) as html_file:
            with open(base_url + rezepte[i].get('href')) as html_file:
                ...
                # write2db(titel, link, bild, thumbnail, zutaten, zubereitung):
...
'''
Bilder nicht als Link in die DB sondern als Bild
